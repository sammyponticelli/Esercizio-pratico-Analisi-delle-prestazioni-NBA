...
data_impact = data[['player_name', 'age_completed_years', 'position', 'team', 'minutes_played',
...
def visualize_impact_dataset(data_impact):
    ...
    data_impact.insert(5, 'games_played',
                       data_impact['wins'].fillna(0) + data_impact['losses'].fillna(0))
    data_impact.insert(6, 'PTS',
                       data_impact['one_point_made']
                       + data_impact['two_point_made'] * 2
                       + data_impact['three_point_made'] * 3)
    data_impact.insert(7, 'REB',
                       data_impact['offensive_rebounds'] + data_impact['defensive_rebounds'])

    data_impact.drop(columns=['one_point_made', 'two_point_made', 'three_point_made',
...
def calculate_per_game(data_impact):
    ...
    data_impact['PTS'] = (data_impact['one_point_made']
                          + data_impact['two_point_made'] * 2
                          + data_impact['three_point_made'] * 3)
    #missing wins/losses count as 0 games played
...
def filter_by_games_played(data_impact):
    ...
    print(f'games floor: {min_games:.1f} (1/3 of {max_games:.0f}) - '
          f'kept {len(filtered)} of {len(data_impact)} players')
...
def impact_score_40_minutes_calculator(data_impact):
    ...
    data_impact['impact_score_40'] = ((data_impact['PTS_average']
                                       + 1.2 * data_impact['REB_average']
                                       + 1.5 * data_impact['AST_average']
                                       + 2 * data_impact['STL_average']
                                       + 2 * data_impact['BLK_average']
                                       - data_impact['TOV_average'])
                                      / minutes * 40).round(2)
...
def impact_score_game(data_impact):
    ...
    data_impact['impact_score_game'] = (data_impact['PTS_average']
                                        + 1.2 * data_impact['REB_average']
                                        + 1.5 * data_impact['AST_average']
                                        + 2 * data_impact['STL_average']
                                        + 2 * data_impact['BLK_average']
                                        - data_impact['TOV_average']).round(2)
...
def cluster_analysis_minutes_impact_score_game(data_impact_game):
    ...
    data_impact_game = data_impact_game.dropna(subset=['minutes_played',
                                                       'impact_score_game']).copy()
...
def compare_top10_game(qualified_dataset):
    average_dataset = qualified_dataset.copy()

    average_dataset['impact_score_game'] = (average_dataset['PTS_average']
                                            + 1.2 * average_dataset['REB_average']
                                            + 1.5 * average_dataset['AST_average']
                                            + 2 * average_dataset['STL_average']
                                            + 2 * average_dataset['BLK_average']
                                            - average_dataset['TOV_average']).round(2)

    average_dataset.drop(columns=['age_completed_years', 'team',
...
def compare_top10_40(qualified_dataset):
    average_dataset = qualified_dataset.copy()

    #0 minutes per game -> NaN instead of dividing by zero
    minutes = average_dataset['MIN_average'].where(average_dataset['MIN_average'] > 0)

    average_dataset['impact_score_40'] = ((average_dataset['PTS_average']
                                           + 1.2 * average_dataset['REB_average']
                                           + 1.5 * average_dataset['AST_average']
                                           + 2 * average_dataset['STL_average']
                                           + 2 * average_dataset['BLK_average']
                                           - average_dataset['TOV_average'])
                                          / minutes * 40).round(2)
...
compare_top10_game(qualified_dataset)
print('\n')
compare_top10_40(qualified_dataset)
...
bar_chart_position_comparison(position_game, position_40)
